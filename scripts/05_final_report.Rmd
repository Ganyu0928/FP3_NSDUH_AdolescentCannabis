...
ex1 <- data_adol %>%
  mutate(RSKMRJWK = ifelse(RSKMRJWK %in% c(85,94,97,98), NA, RSKMRJWK)) %>%
  drop_na(RSKMRJWK)
...
# Define factor labels if needed
analytic$IRSEX <- factor(analytic$IRSEX, 
                         levels = c("Male", "Female"))

analytic$RSKMRJWK <- factor(analytic$RSKMRJWK,
                            levels = c("No risk", "Slight risk", 
                                       "Moderate risk", "Great risk"))

analytic$AGE3 <- factor(analytic$AGE3,
                        levels = c(1,2,3),
                        labels = c("12–13", "14–15", "16–17"))

analytic$YFLMJMO <- factor(analytic$YFLMJMO,
                           levels = c(0,1),
                           labels = c("Neutral/Permissive", "Disapprove"))

analytic$YEPMJMO <- factor(analytic$YEPMJMO,
                           levels = c(0,1),
...
# Table 1
table1(~ AGE3 + IRSEX + RSKMRJWK + YFLMJMO + YEPMJMO + MJDAY30A,
       data = analytic,
       overall = TRUE,
...
```
Interpretation: 
This figure 1. shows a strong inverse relationship between perceived risk and past 30-day marijuana use among adolescents. 
...
```{r}
# this step is to make sure all model has the same size
...
- The odds of use for adolescents perceiving slight risk are 0.25 times the odds among those perceiving no risk, after adjusting for age, sex, peer disapproval, and parental disapproval.
- The odds of use for adolescents perceiving slight risk are 0.08 times the odds among those perceiving no risk, after adjusting for age, sex, peer disapproval, and parental disapproval.
...
Overall, the results highlight the importance of perceived harm as a key driver of adolescent substance-use behavior. Public health efforts that strengthen risk perception, such as school-based education, media campaigns, and family communication strategies, along with strong peer and parent support, can help reduce use.

#5. Reproducibility All data cleaning, descriptive analysis, and
Modeling scripts are stored in the project repository: 
 - 01_data_cleaning.R 
